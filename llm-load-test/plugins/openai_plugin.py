...
class OpenAIPlugin(plugin.Plugin):
    ...
    def request_http(self, query: dict, user_id: int, test_end_time: float = 0):
        ...
        if self.api == "chat":
            request["messages"] = [{"role": "user", "content": query["text"]}]
        ...
        try:
            response = requests.post(
                self.host, headers=headers, json=data, verify=False
            )
    ...
    def streaming_request_http(self, query: dict, user_id: int, test_end_time: float):
        ...
        try:
            for line in response.iter_lines():
                ...
                if line:
                    ...
                    resps.append(dict(time=recv_time, data=line))
        ...
        if message and not message.get("choices") and message.get("usage"):
            ...
            logger.warning(
                "Usage statistics are missing, token count will be inaccurate"
            )
        ...
        for resp in resps:
            ...
            if message.get("error"):
                ...
                logger.error(
                    "Error received in response message: %s", result.error_text
                )
                continue
            ...
            if token["count"] < 1:
                logger.debug(
                    "Omiting response '%s' because it contains %d tokens",
                    token["text"],
                    token["count"],
                )
                continue
        ...
        # the current token.
        result.output_tokens_before_timeout = sum(
            t["count"] for t in tokens if t["time"] <= test_end_time
        )
        ...
        if not result.input_tokens:
            logger.warning(
                "Input token count not found in response, using dataset input_tokens"
            )
        ...
        if expected_output_tokens and result.output_tokens != expected_output_tokens:
            logger.warning(
                f"Received {result.output_tokens} tokens but expected {expected_output_tokens} tokens"
            )

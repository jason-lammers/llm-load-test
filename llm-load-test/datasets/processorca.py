...
class OpenOrcaDatasetGenerator:
    ...
    def filter_output_oob(
        self, df: pd.DataFrame, output_limit: int = 4096
    ) -> pd.DataFrame:
    ...
    def _get_distributed_subset(self, df, step_size: int = 64, rng_seed: int = 1337):
        ...
        for input_lower in range(0, 12288, step_size):
            ...
            for output_lower in range(0, 12288, step_size):
                ...
                data_subset = df[
                    (df["tok_input_length"] > input_lower)
                    & (df["tok_input_length"] < input_upper)
                    & (df["tok_output_length"] > output_lower)
                    & (df["tok_output_length"] < output_upper)
                ]
                ...
                if elements_in_region > 0:
                    ...
                    print(
                        f"In tile: {input_lower}:{input_upper}, {output_lower}:{output_upper}, subset_sample_size: {subset_sample_size}"
                    )
                    # subset_sample=max(1, (len(data_subset))**(1./3.))
                    # sample from the subset
                    sample = data_subset.sample(
                        n=int(subset_sample_size), random_state=rng_seed
                    )
...
def parse_arguments():
    ...
    parser.add_argument(
        "--output_json_file",
        type=str,
        default="openorca_large_subset_011",
        help="Number of samples to generate",
    )
    parser.add_argument(
